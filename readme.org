...
kanjiList is the file where to find. In the default configuration, in the current note, it will find the first field named /Expression/ or /SentenceJp/
as the input, and then replace the field called /heisigana/ with the corresponding result.
